...
class InventoryScreen:
    ...
    SLOT = (110, 113, 108)
    SLOT_BACKGROUND = (54, 46, 40)
    SLOT_HOVER = (91, 74, 60)
    SLOT_BORDER = (166, 132, 85)
    MUTED_TEXT = (220, 210, 190)

    def __init__(
        self,
        inventory_manager: InventoryManager,
        hotbar: Optional[Any] = None,
    ) -> None:
        ...
        # Rectangles from the most recently rendered frame.
        self._bag_slot_rects: List[Tuple[InventorySlot, pygame.Rect]] = []
        self._hotbar_slot_rects: List[Tuple[InventorySlot, pygame.Rect]] = []

        self._selected_slot: Optional[InventorySlot] = None
    ...
    def handle_event(
        ...
        if (
            ...
            bag_slot = self._bag_slot_at(event.pos)

            if bag_slot is not None:

                if not bag_slot.is_empty():

                    self._selected_slot = bag_slot
                    self._selected_item = (
                        bag_slot.get_item()
                    )
            ...
            hotbar_slot = self._hotbar_slot_at(
                event.pos
            )
            ...
            if hotbar_slot is not None:
                ...
                if self._selected_slot is not None:
                    ...
                    print(
                        "[Inventory] Moving item to hotbar"
                    )
    ...
    def draw(self, screen: pygame.Surface, player_sprite: Optional[Tuple[int, int]]= None,) -> None:
        if not self.is_open:
            return
    ...
    def _draw_grid(
        ...
        for index, slot in enumerate(self.inventory_manager.bag_slots):
            ...
            # This exact rendered rectangle is used for click detection.
            self._bag_slot_rects.append((slot, rect))
            self._draw_slot(screen, slot, rect, str(index + 1))
    ...
    def _draw_hotbar(
        ...
        for index, slot in enumerate(self.hotbar.hotbar_slots):
            ...
            self._hotbar_slot_rects.append((slot, rect))
            self._draw_slot(screen, slot, rect, str(index + 1))
    ...
    def _draw_slot(
        ...
        if slot is self._selected_slot:
            colour = self.SLOT_HOVER
        elif rect.collidepoint(pygame.mouse.get_pos()):
            colour = self.SLOT_HOVER
        else:
            colour = self.SLOT_BACKGROUND

        pygame.draw.rect(screen, colour, rect, border_radius=4)
        pygame.draw.rect(screen, self.SLOT_BORDER, rect, 2, border_radius=4)

        item = slot.get_item()
        if item is not None:
            self._draw_item(screen, item, rect, rect.center)
        ...
        if key_label:
            ...
            screen.blit(label, (rect.x + 3, rect.y + 2))
    ...
    def _draw_item(self, screen: pygame.Surface, item: Any, slot_rect: pygame.Rect, center: Tuple[int, int], ghost: bool = False) -> None:
        ...
        if icon:
            ...
            initial = self.label_font.render(str(getattr(item, 'name', '?'))[0].upper(), True, self.PANEL_DARK,)
            screen.blit(initial, initial.get_rect(center=fallback.center))
        quantity = getattr(item, 'quantity', 1)
        if quantity and quantity > 1:
            amount = self.small_font.render(str(quantity), True, self.MUTED_TEXT)
    ...
    def _bag_slot_at(
        ...
        for slot, rect in self._bag_slot_rects:
    ...
    def _hotbar_slot_at(
        ...
        for slot, rect in self._hotbar_slot_rects:
    ...
    def _slot_at_position(
        self,
        position: Tuple[int, int],
    ) -> Optional[InventorySlot]:
        # Test only rectangles that were actually rendered.
        for slot, rect in reversed(self._bag_slot_rects):
            if rect.collidepoint(position):
                return slot

        for slot, rect in reversed(self._hotbar_slot_rects):
            if rect.collidepoint(position):
                return slot
    ...
    def handle_click(
        ...
        clicked_slot = self._slot_at_position(position)

        if clicked_slot is None:
            self._clear_selection()
            return False

        # First click: select only a slot containing an item.
        if self._selected_slot is None:
            item = clicked_slot.get_item()
            if item is None:
                return False

            self._selected_slot = clicked_slot
            self._selected_item = item
            return True

        # Clicking the selected slot again cancels the selection.
        if clicked_slot is self._selected_slot:
            self._clear_selection()
            return True

        # Swap the real slot contents. This also covers moving into an empty hotbar slot.
        selected_item = self._selected_slot.get_item()
        clicked_item = clicked_slot.get_item()

        self._selected_slot.set_item(clicked_item)
        clicked_slot.set_item(selected_item)
